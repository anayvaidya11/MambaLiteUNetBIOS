...
from utils import *
from configs.config_setting_eso import setting_config
...
def main(config):
    ...
    if config.pretrained_path:
        print(f'#----------Loading pretrained weights: {config.pretrained_path}----------#')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained', default=None,
                        help='path to a state dict to fine-tune from, e.g. pre_trained_models/ISIC2018/best-epoch53-loss0.2218.pth')
    parser.add_argument('--tag', default=None,
                        help='label for the results folder, e.g. scratch or finetune')
    args = parser.parse_args()

    config = setting_config
    config.pretrained_path = args.pretrained
    if args.tag is None:
        args.tag = 'finetune' if args.pretrained else 'scratch'
    config.work_dir = ('results/' + config.network + '_' + config.datasets + '_' + args.tag
                       + '_' + datetime.now().strftime('%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/')
    main(config)
